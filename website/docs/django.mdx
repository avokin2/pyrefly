...
these too, as long as both models are in the same module:
...
Since the accessor has no declaration of its own, go-to-definition on it jumps to the
relation field that induced it, which is also where `related_name` is configured.
...
### Cross-Module Reverse Relationships

Reverse relationships are only synthesized when the two models live in the same
module, since Pyrefly discovers them by scanning the module that declares the
relation field:
...
# Not yet supported: Reporter comes from another module
reporter = Reporter()
reveal_type(reporter.article_set)  # Expected: RelatedManager[Article]
```

Relatedly, Pyrefly does not track `Meta.abstract`. A relation field on an abstract
base class gets its reverse accessor named after the base rather than after each
concrete subclass, so the accessor Pyrefly offers may not be the one Django creates.
