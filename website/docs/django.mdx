...
- Handles relationships between models (`ForeignKey`, `ManyToManyField`), in both directions — including reverse accessors like `question.choice_set`
...
Django adds an accessor to the model a relation field points at. Pyrefly synthesizes
these too:
...
question = Question()
assert_type(question.choice_set, RelatedManager[Choice])
assert_type(question.choice_set.all(), QuerySet[Choice, Choice])
```

The accessor is named after the source model — `<model_lowercase>_set` for
`ForeignKey` and `ManyToManyField`, and just `<model_lowercase>` for a
`OneToOneField`, which yields a single instance rather than a manager. A
`related_name` argument overrides that name, and `related_name='+'` removes the
accessor entirely.

Because a relation can be declared anywhere in the project, Pyrefly scans your
project's files for relation fields so that it knows which modules to consult. Two
things fall outside what that scan can see, and no accessor is synthesized for them:

```python
MyAlias = Question

class Choice(models.Model):
    # The target is an alias, so it cannot be matched by name.
    question = models.ForeignKey(MyAlias, on_delete=models.CASCADE)

class Vote(models.Model):
    # The target is passed by keyword rather than positionally.
...
### Reverse Relationships From Abstract Bases

Pyrefly does not track `Meta.abstract`. A relation field on an abstract base class
gets its reverse accessor named after the base rather than after each concrete
subclass, so the accessor Pyrefly offers may not be the one Django creates.

### Reverse Relationships From Installed Packages

The scan that finds relation fields covers your project's files, not installed
packages. A model in a third-party app that points at one of your models therefore
adds no reverse accessor.
