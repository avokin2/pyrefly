...
Every manager and queryset carries its model as a type argument, so the model survives `filter()`
chains, indexing and method calls without any further annotation:
...
assert_type(Article.objects.all().order_by("title")[0], Article)
```

#### Custom Managers

A manager declared in a model body belongs to that model, but a manager class names no model of its
own. Pyrefly supplies the missing binding from the enclosing model:
...
assert_type(Article.objects.published(), QuerySet[Article, Article])
```

A manager written without a type parameter cannot be spelled as `ArticleManager[Article]`. Pyrefly
keeps both halves — the manager's own methods and the model-parameterized inherited ones:
...
assert_type(Article.objects.published(), str)   # the manager's own method
assert_type(Article.objects.get(), Article)     # inherited, bound to Article
```

An explicit parameter is always respected: `authors = ArticleManager[Author]()` stays
`ArticleManager[Author]` rather than being rebound to the enclosing model.

#### `as_manager()`

`QuerySet.as_manager()` builds a manager that copies the queryset's methods. The stub signature
returns a plain `Manager[_Model]`, so Pyrefly restores the queryset:
...
Note that Django skips methods marked `queryset_only = True` when building the manager; Pyrefly does
not model that, so such methods still appear.
...
`Manager.from_queryset(MyQuerySet)` is declared as returning `type[Self]`, which erases the concrete
queryset before the assignment is seen. The model is still recovered, but the queryset's own methods
are not:
...
reveal_type(Article.objects.get())        # Article, as expected
reveal_type(Article.objects.published())  # Not yet supported
```

Prefer `ArticleQuerySet.as_manager()`, which names the queryset at the call site and is fully supported.

### Managers on Abstract Models

A manager declared on an abstract base binds to that base rather than to each concrete subclass, so
`Article.objects` is `ArticleManager[Base]` instead of `ArticleManager[Article]`.
